...
int main()
{
    ...
    // pthreads sort competition
    if(pthread_create(&bubble_thread, NULL, bubble_sort_run, array_1)) 
    {
        fprintf(stderr, "Error creating thread\n");
        return 1;
    }
    if(pthread_create(&select_thread, NULL, select_sort_run, array_2)) 
    {
        fprintf(stderr, "Error creating thread\n");
        return 1;
    }
    if(pthread_create(&binary_thread, NULL, binary_sort_run, array_3)) 
    {
        fprintf(stderr, "Error creating thread\n");
        return 1;
    }
    // join
    if(pthread_join(bubble_thread, NULL)) 
    {
    ...
    if(pthread_join(binary_thread, NULL)) 
    {
        fprintf(stderr, "Error joining thread\n");
        return 2;
    }
